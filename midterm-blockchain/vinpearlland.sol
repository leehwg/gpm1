...
contract Vinpearl{
    ...
    // Data structure to save customers
    struct customer {
        uint buyed_tokens;
    ...
    function buyTokens(uint _tokensAmount) public payable {
        ...
        // Vinpearl land returns ether amount to the customer
    ...
    // Modifier to control executable functions only by Vinpearl land
    ...
    // ------------------------------- VINPEARL LAND ATRACTIONS MANAGEMENT -------------------------------
    ...
    // Mapping to match a customer with his attractions record in Vinpearl land
    mapping(address => string []) AttractionsRecord;

    // Create new Vinpearl land attraction executable only by (owner)
    ...
    // Visualize available attractions
    function visualizeAvailableAttractions() public view returns(string[] memory) {
        return Attractions;
    }
    ...
    function takeAttraction(string memory _attractionName) public {
        ...
        /* 
            Transfer customer's tokens to Vinpearl land address.
            To archive this, we had to create a special function called transferVinpear land() in ERC20.sol contract
            because transfer() function take the contract's address as a sender, and we needed to change it for customer's address.
        */
        token.transferVinpearl(msg.sender, address(this), attraction_tokens);
    ...
    // Give Ether to a customer that returns unused Vinpearl land tokens
    function returnTokens(uint _tokensAmount) public payable {
        ...
        // Customer returns his tokens to Vinpearl land
        token.transferVinpearl(msg.sender, address(this), _tokensAmount);
        // Vinpearl land returns Ether to the customer
        msg.sender.transfer(tokenPrice(_tokensAmount));
    }


    // ------------------------------- VINPEARL LAND FOOD MANAGEMENT -------------------------------
    ...
    // Mapping to match a customer with his dishes record in Vinpearl land
    mapping(address => string []) DishesRecord;

    // Create new dish in the menu executable only by owner
    ...
    function buyDish(string memory _dishName) public {
        ...
        /* 
            Transfer customer's tokens to Vinpearl land address.
            To archive this, we had to create a special function called transferVinpearl() in ERC20.sol contract
            because transfer() function take the contract's address as a sender, and we needed to change it for customer's address.
        */
        token.transferVinpearl(msg.sender, address(this), dish_tokens);
